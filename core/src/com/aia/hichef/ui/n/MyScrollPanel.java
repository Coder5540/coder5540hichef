...
import com.aia.hichef.listener.ScrollListenner;
...
public class MyScrollPanel extends WidgetGroup {
	public static int				speed				= 17;
	public boolean					isFreeMove			= true,
			isCanTouch = true;

	private MyScrollPanelStyle		style;
	...
	float							fadeAlpha, fadeAlphaSeconds = 1, fadeDelay,
			fadeDelaySeconds = 1;
	boolean							cancelTouchFocus	= true;

	boolean							flickScroll			= true;
	float							velocityX, velocityY, vFlingX;
	float							flingTimer;
	private boolean					overscrollX			= false,
			overscrollY = false;
	float							flingTime			= 1f;
	private float					overscrollDistance	= 80,
			overscrollSpeedMin = 30, overscrollSpeedMax = 200;
	...
	public ScrollListenner			scrollListenner;

	/**
	 * @param widget
	 *            May be null.
	 */
	public MyScrollPanel(Actor widget) {
		this(widget, new MyScrollPanelStyle());
	}
	 ...
	 *            May be null.
	 */
	public MyScrollPanel(Actor widget, Skin skin) {
		this(widget, skin.get(MyScrollPanelStyle.class));
	}
	 ...
	 *            May be null.
	 */
	public MyScrollPanel(Actor widget, Skin skin, String styleName) {
		this(widget, skin.get(styleName, MyScrollPanelStyle.class));
	}
	 ...
	 *            May be null.
	 */
	public MyScrollPanel(Actor widget, MyScrollPanelStyle style) {
		if (style == null)
		...
		addCaptureListener(new InputListener() {
			...
			public boolean touchDown(InputEvent event, float x, float y,
				...
				if (pointer == 0 && button != 0)
					return false;
				getStage().setScrollFocus(MyScrollPanel.this);
		...
		flickScrollListener = new ActorGestureListener() {
			public void pan(InputEvent event, float x, float y, float deltaX,
					float deltaY) {
				MyScrollPanel.this.pan(event, x, y, deltaX, deltaY);
	...
	void clamp() {
		...
		if (scrollListenner != null) {
			if (!disableX) {
				if (amountX < 0 && getX() == 0)
					scrollListenner.onOverLeft(amountX);
				if (amountX > maxX)
					scrollListenner.onOverRight(amountX);
			}
			if (!disableY) {
				if (amountY > maxY)
					scrollListenner.onOverDown(amountY);
				if (amountY < 0)
					scrollListenner.onOverUp(amountY);
			}
		}
	...
	public void setStyle(MyScrollPanelStyle style) {
		if (style == null)
			throw new IllegalArgumentException("style cannot be null.");
		this.style = style;
		invalidateHierarchy();
	 ...
	 * have an effect until {@link #setStyle(MyScrollPanelStyle)} is called.
	 */
	public MyScrollPanelStyle getStyle() {
		return style;
	}
	...
	public void setWidget(Actor widget) {
		if (widget == this)
			throw new IllegalArgumentException(
					"widget cannot be the MyScrollPanel.");
		if (this.widget != null)
	...
	/**
	 * @deprecated MyScrollPanel may have only a single child.
	 * @see #setWidget(Actor)
	 */
	public void addActor(Actor actor) {
		throw new UnsupportedOperationException("Use MyScrollPanel#setWidget.");
	}
	 ...
	 * FlickMyScrollPanel in the disabled direction.
	 */
	 ...
	 * MyScrollPanel that have received touchDown to receive touchUp when flick
	 * scrolling begins.
	 */
	...
	/**
	 * The style for a scroll pane, see {@link MyScrollPanel}.
	 * 
	 * @author mzechner
	 * @author Nathan Sweet
	 */
	static public class MyScrollPanelStyle {
		/** Optional. */
		...
		public MyScrollPanelStyle() {
		}

		public MyScrollPanelStyle(Drawable background, Drawable hScroll,
				Drawable hScrollKnob, Drawable vScroll, Drawable vScrollKnob) {
		...
		public MyScrollPanelStyle(MyScrollPanelStyle style) {
	...
	public void addScrollListener(ScrollListenner listener) {
		this.scrollListenner = listener;
	}
